    ...
    <c:forEach var="sp" items="${listSP}" varStatus="loop">
        <div class="col-sm-4">
            <tr>
                ...
                <td>${sp.diachi}</td>
                <td>${sp.birthday}</td>
                <td style="width: 100px; height: 100px"><a href="/login?action=createSP&id=${user.id}" --%>
                    class="btn btn-success">Delete</a></td>
                <td style="width: 100px; height: 100px"><a href="/login?action=editSP&id=${user.id}" --%>
                    class="btn btn-success">Delete</a></td>
                <td style="width: 100px; height: 100px"><a href="/login?action=deleteSP&id=${user.id}"
                                                           class="btn btn-success">Delete</a></td>
            </tr>
        </div>
    </c:forEach>
